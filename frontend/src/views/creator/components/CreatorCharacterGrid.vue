...
/**
 * 创作者旗下代表作角色卡 2x2 网格组件 (1:1 原型高保真)
 *
...
defineProps<{
  works: CreatorCharacterWork[];
}>();

const emit = defineEmits<(e: "select-work", work: CreatorCharacterWork) => void>();
</script>
    ...
    >
      ...
      <div class="relative w-full h-32 overflow-hidden bg-[#292524]">
        ...
        <div class="absolute top-2 right-2 z-10 flex flex-col items-end gap-1">
          <!-- 火热指数 (火焰图标 + 数值) -->
          <div class="flex items-center gap-1 bg-black/50 px-1.5 py-0.5 rounded backdrop-blur-xs">
            ...
            <span class="text-xs font-bold text-[#EAB308]">{{ work.heat }}</span>
          </div>

          <!-- 杯赛排名 (若有) -->
          <div
            v-if="work.rankBadge"
            class="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-[#F9C86D]/20 border border-[#F9C86D]/40 text-[10px] font-semibold text-[#F9C86D] shadow-sm"
          >
            <span>{{ work.rankBadge.cupName }}</span>
            <span class="font-bold">{{ work.rankBadge.rank }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="p-2.5 flex flex-col gap-1.5 justify-between flex-1">
        ...
        <div class="flex items-center gap-2 text-[10px] text-[#FF9F43]">
          ...
          <div class="flex items-center gap-0.5">
            ...
            <span>{{ work.tokenUsage }}</span>
          </div>

          <!-- 评分 -->
          <div class="flex items-center gap-0.5 text-[#F9C86D]">
            ...
            <span class="font-bold">{{ work.rating }}</span>
          </div>
        </div>
