...
/**
 * 个人中心 - 创作者等级卡片 (1:1 原型高保真)
 *
 * @packageDocumentation
 */

import { CREATOR_LEVEL_DATA } from "@/views/profile/constants/profileMock";
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <span class="text-[14px] font-semibold text-[#78716C] leading-5">
            {{ CREATOR_LEVEL_DATA.title }}
          </span>
          <span class="text-xs text-[#78716C] font-medium">
            等级 {{ CREATOR_LEVEL_DATA.level }}
          </span>
        </div>
        <button
          type="button"
      ...
      <div class="flex flex-col gap-1 mt-3">
        <div class="flex items-center justify-between text-xs">
          ...
          <span class="text-[#A8A29E] font-mono font-medium">
            {{ CREATOR_LEVEL_DATA.currentXp }} / {{ CREATOR_LEVEL_DATA.nextLevelXp }} XP
          </span>
        </div>
        <div class="w-full h-2 rounded-full bg-[#292524] overflow-hidden relative">
          <div
            ...
            :style="{ width: `${CREATOR_LEVEL_DATA.progressPercentage}%` }"
          />
        </div>
        <p class="text-[11px] text-[#78716C] text-right mt-0.5">
          {{ CREATOR_LEVEL_DATA.progressPercentage }}% 至下一等级
        </p>
      </div>
    </div>

    <!-- 3. 5 项指标阵列 (总经验、原创作品、作品使用、奖励星元、奖励月华) -->
    <div class="w-full grid grid-cols-5 gap-1 pt-3 mt-1 border-t border-[#44403C]/60 text-center">
      <!-- (1) 总经验 -->
      <div class="flex flex-col items-center">
        <span class="text-[13px] font-bold text-[#F5F5F4] font-mono tracking-tight">{{ CREATOR_LEVEL_DATA.totalXp }}</span>
        <span class="text-[10px] text-[#78716C] mt-0.5">总经验</span>
      </div>

      <!-- (2) 原创作品 -->
      <div class="flex flex-col items-center">
        <span class="text-[13px] font-bold text-[#F5F5F4] font-mono tracking-tight">{{ CREATOR_LEVEL_DATA.originalWorks }}</span>
        <span class="text-[10px] text-[#78716C] mt-0.5">原创作品</span>
      </div>

      <!-- (3) 作品使用 -->
      <div class="flex flex-col items-center">
        <span class="text-[13px] font-bold text-[#F5F5F4] font-mono tracking-tight">{{ CREATOR_LEVEL_DATA.worksUsage }}</span>
      ...
      <div class="flex flex-col items-center">
        <div class="flex items-center gap-0.5">
          ...
          <span class="text-[13px] font-bold text-[#F5F5F4] font-mono tracking-tight">{{ CREATOR_LEVEL_DATA.rewardStars }}</span>
        </div>
      ...
      <div class="flex flex-col items-center">
        <div class="flex items-center gap-0.5">
          ...
          <span class="text-[13px] font-bold text-[#F5F5F4] font-mono tracking-tight">{{ CREATOR_LEVEL_DATA.rewardMoons }}</span>
        </div>
