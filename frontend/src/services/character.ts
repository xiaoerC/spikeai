...
export const characterService = {
  /** 获取角色市场列表 */
  async getCharacters(params: CharacterFilterParams = {}): Promise<PaginatedData<CharacterListItem>> {
    const res = await api.get<ApiResponse<PaginatedData<CharacterListItem>>>('/characters', { params })
  ...
  async toggleLike(id: string): Promise<{ is_liked: boolean; like_count: number }> {
    const res = await api.post<ApiResponse<{ is_liked: boolean; like_count: number }>>(`/characters/${id}/like`)
    return res.data.data
  },

  /** 获取角色评论列表 */
  async getComments(id: string, page = 1, pageSize = 20): Promise<PaginatedData<CharacterComment>> {
    const res = await api.get<ApiResponse<PaginatedData<CharacterComment>>>(`/characters/${id}/comments`, {
  ...
  async addComment(id: string, content: string): Promise<CharacterComment> {
    const res = await api.post<ApiResponse<CharacterComment>>(`/characters/${id}/comments`, { content })
    return res.data.data
  },
  ...
  async rewardCharacter(
    ...
    const res = await api.post<ApiResponse<{ remaining_balance: number }>>(`/characters/${id}/reward`, {
      currency,
      amount,
    })
    return res.data.data
  ...
  async updateCharacterStatus(
    ...
    const res = await api.patch<ApiResponse<CharacterDetail>>(`/characters/${id}/status`, { status })
    return res.data.data
  },
