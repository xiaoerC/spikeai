# 叙梦 Naro (SpikeAI) 全景工程落地路线图与后续分期规划 v5.0.0
...
### 2. 待执行深度规划明细 (Phase 7 ~ Phase 10)

---

#### 🧩 **Phase 7: Mod 优先级加载矩阵与多级指令扩展流水线 (🌟 下一个立即执行的核心目标)**

- **Step 7.1: Mod 领域数据模型 (`models/mod.py`)**：
  - `prompt_mods`：包含 `name`, `description`, `category` (system/jailbreak/persona/visual/tool), `target_anchor` (`system_prefix` / `before_char` / `after_char` / `top_an` / `bottom_an` / `user_suffix`), `content`, `priority` (数值越小优先级越高), `is_global`, `is_enabled`；
  - `session_mod_bindings`：会话级 Mod 启停绑定与参数覆盖；
- **Step 7.2: 多锚点 Prompt 插桩流水线 (`services/mod_pipeline_service.py`)**：
  - 定义标准槽位：`SYSTEM_PREFIX` -> `CHARACTER_CORE` -> `WORLD_BOOK` -> `CONTROL_PANEL` -> `NARRATIVE_STATE` -> `MOD_POST_PROCESSING` -> `CHAT_HISTORY` -> `USER_SUFFIX`；
  - 自动按优先级排序合并多 Mod 提示词；
- **Step 7.3: RESTful API 与预设库**：
  - `/api/v1/mods`：CRUD、官方预设 Mod 库（二次元画师串、沉浸式对话增强、越狱破限、RPG 战斗系统）；
  - `/api/v1/chat/sessions/{session_id}/mods`：会话 Mod 绑定与启停切换；
- **Step 7.4: 前端黑金 Mod 管理抽屉 (`ModManagerDrawer.vue`)**：
  - 抽屉内支持卡片拖拽调整优先级、一键启停开关、自定义 Mod 编辑 Modal；
  - 底部输入栏快捷开关与当前生效 Mod Badge 展示；
- **Step 7.5: 质量门禁与单测**：
  - 编写 `tests/test_mod_pipeline.py`，全量单测覆盖多 Mod 排序与锚点插桩逻辑。

---

#### ⚡ **Phase 8: 60fps 平滑打字机与 IndexedDB 离线秒开加速引擎**
